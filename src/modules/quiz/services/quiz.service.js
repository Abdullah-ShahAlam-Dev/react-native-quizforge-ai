// quiz.service.js
// Simulates a call to the Python AI backend that generates MCQs.
// Production: replace with  api.post('/quiz/generate', { topic, difficulty, numQuestions })
...
// ─── Helper ─────────────────────────────────────────────────────────
const getQuestions = (topic, numQuestions) => {
...
export const generateQuiz = async (topic, difficulty, numQuestions) => {
  ...
  // return response.data; // expected: { questions: [...] }

  // ── MOCK ──
  return new Promise((resolve) => {
    setTimeout(() => {
      const questions = getQuestions(topic, numQuestions);
      resolve({ questions, topic, difficulty, numQuestions });
    }, 1800); // simulates AI generation latency
  });
};